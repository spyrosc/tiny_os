...
TCB* spawn_thread(PCB* pcb, void (*func)(),PTCB* parent_ptcb)
{
  ...
  tcb->owner_PTCB=parent_ptcb; //bazoume pointer sto PTCB tou thread
...
static TCB* sched_queue_select()
{
  ...
  int k=0;
  rlnode* sel = rlist_pop_front(& SCHED[k]);
   while(sel->tcb==NULL && k<PRIORITY_QUEUES)
    { 
      k++;
      sel = rlist_pop_front(& SCHED[k]);
    }
 ...
 */
// An einai STOPPED h INIT balto ston scheduler alliws return 0 
...
{ 
  ...
  TCB* current = CURTHREAD;  /* Make a local copy of current process, for speed */
    ...
    int current_ready = 0;
    rlnode* intermediate_node;
     yield_counter++;

    Mutex_Lock(& sched_spinlock);
     //if((now-(current->time_passed))<(QUANTUM-(1000L)) && current->priority>0)
     //current->priority--;

      if(cause==SCHED_QUANTUM && current->priority<(PRIORITY_QUEUES-1))
      current->priority++;
      else if(cause==SCHED_IO && current->priority>0)
      current->priority=0;
      else if(cause==SCHED_MUTEX && current->priority<(PRIORITY_QUEUES-1))
      current->priority++;
      ...
      { 
        yield_counter=0;
        for(int j=2;j<PRIORITY_QUEUES;j++)
        {
           while(!is_rlist_empty(&SCHED[j]))
           {
             intermediate_node=rlist_pop_front(&SCHED[j]);
             rlist_push_back(&SCHED[j-1],intermediate_node);
           }
...
void gain(int preempt)
{
  ...
  current->time_passed=bios_clock();
